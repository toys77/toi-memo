...
v1.4.2ではAdobe Fontsをロゴ用に整理しました。`index.html` の `<head>` 内でkit `nyu7vux` を読み込み、`style.css` では次のfont-familyを確認済みです。
...
開発確認用に、ブラウザのコンソールへ `document.documentElement.className` を出力します。`wf-active` ならAdobe Fontsが有効、`wf-inactive` なら読み込み失敗またはタイムアウトです。
