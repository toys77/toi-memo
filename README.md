...
v2.0.1では、メモ一覧上部にホームダッシュボードを追加しました。総メモ数、今日作成したメモ数、7日以内に更新したメモ数、ピン留め数、お気に入り数、カテゴリ別メモ数を確認できます。
...
v2.0.1では、メモ一覧を `カード`、`リスト`、`コンパクト` の3つの表示モードで切り替えられるようになりました。最後に選んだ表示モードは `toiMemo.viewMode` に保存され、次回起動時にも復元されます。
